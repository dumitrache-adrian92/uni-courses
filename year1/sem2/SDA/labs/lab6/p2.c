...
void valoareMaxima(struct nod *radacina, int *max) {
    if (radacina != NULL)
    {
        if (radacina->valoare > *max)
            *max = radacina->valoare;
        valoareMaxima(radacina->stanga, max);
        valoareMaxima(radacina->dreapta, max);
    }
}
...
int main()
{
    ...
    int maxSt = 0, maxDr = 0;
    valoareMaxima(radacina->stanga, &maxSt);
    valoareMaxima(radacina->dreapta, &maxDr);
    printf("%d %d\n", maxSt, maxDr);
    return 0;
}