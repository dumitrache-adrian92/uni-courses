...
def compute_shared_key(other_key):
    ...
    print("SERVER: shared key:", shared_key, flush=True)
...
def send_func(sock, packet, addr):
    print("SERVER: send", packet, addr, flush=True)
...
def receive_func(sock):
    while True:
        ...
        print("SERVER: got packet", packet_type, packet_data, flush=True)

        if packet_type == 'KEY':
            global dh
            send_func(sock, assemble_packet('KEY', dh.get_public_key(), True), from_addr)
            ...
            if decrypted.startswith("GIFF_ME_FLAG"):
                send_func(sock, assemble_packet('MSG', flag.encode(), False), from_addr)
