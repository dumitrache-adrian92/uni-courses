...
def compute_shared_key(other_key):
    ...
    print("CLIENT: shared key:", shared_key, flush=True)
...
def send_func(sock, packet, addr):
    print("CLIENT: send", packet, addr, flush=True)
...
def receive_func(sock):
    while True:
        ...
        print("CLIENT: got packet", packet_type, packet_data, flush=True)

        if packet_type == 'KEY':
            # if message is a key message, compute the shared key
            compute_shared_key(packet_data)
            # send request
            send_func(sock, assemble_packet('MSG', "GIFF_ME_FLAG".encode(), False), (host, server_port))
        elif packet_type == 'MSG':
            print(decrypt_message(packet_data), flush=True)  # else print the message


def send_request(sock):
    while True:
        global dh
        # final key is going to be 1024 bits long (group 2 means 1024 bits prime)
        # send public key to receiver to compute shared key
        send_func(sock, assemble_packet('KEY', dh.get_public_key(), True), (host, server_port))
        time.sleep(30)
...
def main():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
        ...
        threaded_send_flag = threading.Thread(target=send_request, args=(sock,))
        threaded_send_flag.start()
        threaded_receive.join()
        threaded_send_flag.join()


if __name__ == '__main__':
    if len(sys.argv) > 1 and sys.argv[1] == "--mitm":
        print("Using server port 1337!", flush=True)
        server_port = 1337
    main()
