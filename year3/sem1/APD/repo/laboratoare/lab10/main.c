...
int* get_dst(int rank, int numProcs, int leader) {
	...
	*/
	...
	for (int i = 0; i < num_neigh; i++) {
		if (neigh[i] != v[rank]) {
			MPI_Send(vRecv, numProcs, MPI_INT, neigh[i], 0, MPI_COMM_WORLD);
		}
	}
	...
	for (int k = 0; k < CONVERGENCE_COEF; k++) {
		...
		*/
		...
		for (int i = 0; i < nProcesses; i++) {
			...
			int neigh_leader;
			MPI_Recv(&neigh_leader, 1, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			leader = neigh_leader > leader ? neigh_leader : leader;
		}
	}
