						  ...
						  char *filename)
{
	...
	while (1)
	{
		...
		do {
			...
			rc = sendto(sockfd, &d, sizeof(d), 0, (struct sockaddr *)&server_address,
			sizeof(server_address));

			/* TODO 1.3: Wait for ACK before moving to the next datagram to send. If timeout, resend the datagram. */
			int rc = recvfrom(sockfd, &recvd, sizeof(struct seq_udp), 0,
						 ...
						 char *filename)
{
	...
	int rc;
	int i = 0;
	struct cel *current = window->head;
