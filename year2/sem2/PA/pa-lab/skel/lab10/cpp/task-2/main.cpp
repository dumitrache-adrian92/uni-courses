...
private:
    ...
    JohnsonResult compute() {
        ...
        auto BLF_result = bellman(n + 1, n + 1, edges);

        if (BLF_result.has_cycle)
            return {true, d, p};
        ...
        for (int i = 1; i <= n; i++)
            for (auto edge : adj[i])
                d[i][edge.first] = edge.second + (h[i] - h[edge.first]);

        for (int i = 1; i <= n; i++) {
            auto dj_result = dijkstra(i, n, adj);

            for (int j = 1; j <= n; j++) {
                d[i][j] = dj_result.d[j] + (h[j] - h[i]);
                p[i][j] = dj_result.p[j];
            }
        }
