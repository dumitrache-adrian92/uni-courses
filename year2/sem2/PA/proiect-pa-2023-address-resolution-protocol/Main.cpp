...
class EngineComponents {
 ...
 public:
  ...
  void leaveForceMode() {
    ...
    Move *move = bot.value()->calculateNextMove(engineSide);
    emitMove(move);
  ...
  void processIncomingMove(Move *move) {
    if (state.value() == FORCE_MODE) {
      /* Primeste o miscare de la user, o proceseaza */
      bot.value()->recordMove(move, sideToMove, engineSide);
      toggleSideToMove();
      ...
      Move *response = bot.value()->calculateNextMove(engineSide);
      emitMove(response);
