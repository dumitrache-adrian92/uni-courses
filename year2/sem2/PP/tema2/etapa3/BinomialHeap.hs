...
attach t1@(Node p1 k1 children1) t2@(Node p2 k2 children2)
    | p1 < p2 = Node p1 k1 (t2 : children1)
    | p2 < p1 = Node p2 k2 (t1 : children2)
    | otherwise = Node p1 k1 (t2 : children1)

attach EmptyTree EmptyTree = EmptyTree

attach a@(Node x y z) EmptyTree = a
attach EmptyTree a@(Node x y z) = a
...
insertTree tree@(Node p k children) trees = case trees of
    EmptyTree : xs -> tree : xs
    first@(Node p' k' children') : xs -> EmptyTree : insertTree (attach first tree) xs
    [] -> [tree]
...
findMin heap@(BinomialHeap size trees) =
    let
        nonNullTrees = [tree | tree@(Node a b c) <- trees]
        getPrioKeyPair tree@(Node p k children) = (p, k)
    in
        if null nonNullTrees
            then Nothing
        else Just $ getPrioKeyPair $ minimumBy (compare `on` (\tree@(Node p k children) -> p)) nonNullTrees

--minimumBy (compare `on` (\tree@(Node p k children) -> p)) [tree | tree <- trees, not (null tree)]
...
mergeTrees trees1 trees2 =
    let
        ...
        mergeTreesWithCarry carry pair = case (pair, carry) of
            ((EmptyTree, EmptyTree), EmptyTree) -> (EmptyTree, EmptyTree)
            ((Node a b c, EmptyTree), EmptyTree) -> (EmptyTree, Node a b c)
            ((EmptyTree, Node a b c), EmptyTree) -> (EmptyTree, Node a b c)
            ((x@(Node a b c), y@(Node d e f)), EmptyTree) -> (attach x y, EmptyTree)
            ((x@(Node a b c), y@(Node d e f)), z@(Node g h i)) -> (attach x y, z)
            ((EmptyTree, y@(Node d e f)), z@(Node g h i)) -> (attach z y, EmptyTree)
            ((x@(Node a b c), EmptyTree), z@(Node g h i)) -> (attach z x, EmptyTree)
            ((EmptyTree, EmptyTree), z@(Node g h i)) -> (EmptyTree, z)
        pair = mapAccumL mergeTreesWithCarry EmptyTree zippedTrees
        carry = fst pair
        result = snd pair
...
-}
isolate :: a -> [a] -> [(a, [a])]
isolate placeHolder list = snd $ mapAccumL replace 0 list
    where
        replaceAtIndex list index placeHolder =
            if index == length list - 1
            then (head $ drop index list, take index list)
            else (head $ drop index list, take index list ++ [placeHolder] ++ drop (index + 1) list)
        replace acc elem = (acc + 1, replaceAtIndex list acc placeHolder)
...
removeMin heap@(BinomialHeap size trees) =
    let
        isolatedTrees = isolate EmptyTree trees
        nonNullIsolatedTrees = [x | x@(Node a b c, list) <- isolatedTrees]
        mergeOrphans (tree@(Node p k children), list) = mergeTrees list (reverse children)
    in
        if
            null nonNullIsolatedTrees
        then
            heap
        else
            BinomialHeap (size - 1) (mergeOrphans $ minimumBy (compare `on` (\(tree@(Node p k children), list) -> p)) nonNullIsolatedTrees)
...
helper filler level EmptyTree = "*"
helper filler level tree@(Node prio key []) =
    replicate (level * 2) filler ++ show prio ++ " (" ++ show key ++ ")"
helper filler level tree@(Node prio key children) =
    replicate (level * 2) filler ++ show prio ++ " (" ++ show key ++ ")\n" ++ intercalate "\n" (map (helper filler (level + 1)) children)

instance (Show p, Show k) => Show (BinomialTree p k) where
    show tree@(Node prio key children) = helper ' ' 0 tree
    show EmptyTree = "*"
...
instance Foldable (BinomialTree p) where
    ...
    foldr f acc tree@(Node prio key children) = k $ foldr (.) id (getListOfPartialApplication c) acc
        where
            partialApply@(Node p k c) = fmap f tree
            getListOfPartialApplication = foldr (\current@(Node p k c) acc -> k : (getListOfPartialApplication c ++ acc)) []