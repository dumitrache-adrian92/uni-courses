...
(define (match person engagements pref1 pref2 queue)
  (if (equal? person #f)
      engagements
      (let parse-prefs ((prefs (get-pref-list pref1 person)))
        (if (null? prefs)
            (cons (cons #f person) engagements)
            (let* ((potential-mate (car prefs)) (potential-mate-partner (get-partner engagements potential-mate)))
              (if (member potential-mate queue)
                  (parse-prefs (cdr prefs))
                  (if (or (equal? potential-mate-partner #f)  (preferable? (get-pref-list pref2 potential-mate) person potential-mate-partner))
                      (match potential-mate-partner (update-engagements engagements potential-mate person) pref1 pref2 queue)
                      (parse-prefs (cdr prefs))
                      )
                  )
              )
...
(define (path-to-stability engagements mpref wpref queue)
  (let parse-queue ((remaining-queue queue) (result engagements))
    (if (null? remaining-queue)
        result
        (if (member (car remaining-queue) (get-men mpref))
            (parse-queue (cdr remaining-queue) (match (car remaining-queue) result mpref wpref (cdr remaining-queue)))
            (parse-queue (cdr remaining-queue) (reverse-engagements (match (car remaining-queue) (reverse-engagements result) wpref mpref (cdr remaining-queue))))
            )
        )
    )
  )
...
; - fiecare cuplu din lista engagements are pe prima poziție
;   o femeie
(define (room-engagements engagements unstable)
  (filter (λ (couple) (not (member couple unstable))) engagements)
  )

(define (update-stable-match engagements mpref wpref)
  (let ((unstable (get-unstable-couples engagements mpref wpref)))
    (path-to-stability (room-engagements engagements unstable) mpref wpref (get-couple-members unstable))
    )
  )
...
(define (generate-engs pref-stream prev-engs)
  (if (stream-empty? pref-stream)
      empty-stream
      (let* ((first-elem (stream-first pref-stream)) (current-engs (update-stable-match prev-engs (car first-elem) (cdr first-elem))))
        (stream-cons current-engs
                     (generate-engs (stream-rest pref-stream) current-engs)
                     )
        )
      )
  )
...
(define (build-stable-matches-stream pref-stream)
  (if (stream-empty? pref-stream)
      empty-stream
      (let* ((first-elem (stream-first pref-stream)) (engagements (gale-shapley (car first-elem) (cdr first-elem))))
        (stream-cons engagements
                     (generate-engs (stream-rest pref-stream) engagements)
                     )
        )
      )
  )