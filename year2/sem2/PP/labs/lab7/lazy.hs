...
findStringsLongerThanTenChars l = filter (\string -> if (length string) >= 10 then True else False) l

findStringsLongerThanTenChars2 :: [String] -> [String]
findStringsLongerThanTenChars2 l = [string | string <- l, (length string) >= 10]
...
buildPairsStringLength l = map (\string -> (string, length string)) l
...
newtonRaphson x g dg = infiniteApply (\x_old -> x_old - (g x_old) / (dg x_old)) x
...
newtonRaphsonSolve x_old atol g dg =
    let
        newtonRaphsonStream = newtonRaphson x_old g dg
        newtonRaphsonStreamZipped = zip (tail newtonRaphsonStream) newtonRaphsonStream
    in snd $ head $ dropWhile (\(x, y) -> abs(x - y) > atol) newtonRaphsonStreamZipped
...
babylonianMethod a x_old atol = let
        babylonianMethodStream = newtonRaphson x_old (\x -> x**2 - a) (\x -> 2 * x)
        babylonianMethodStreamZipped = zip (tail babylonianMethodStream) babylonianMethodStream
    in snd $ head $ dropWhile (\(x, y) -> abs(x - y) > atol) babylonianMethodStreamZipped
