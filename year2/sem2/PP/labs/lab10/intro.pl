...
myConcat([],L,L).
myConcat([First|Rest1], L2, [First|RestResult]) :- myConcat(Rest1, L2, RestResult).
...
myReverse([],[]) :- true.
myReverse([Elem|Rest], RevList) :- myReverse(Rest, RevRest), myConcat(RevRest, [Elem], RevList).
...
myReverseAcc([], L, L).
myReverseAcc([HL|TL], Acc, RevList) :- myReverseAcc(TL, [HL|Acc], RevList).
...
factorial(0, 1).
factorial(N, Fact) :- N > 0, N1 is N-1, factorial(N1, FactLess), Fact is FactLess * N.
...
palindrom(L) :- myReverse(L, LRev), L = LRev.
...
descendantOf(X,Y):- arc(Y, X); arc(Y, Descendant), descendantOf(X, Descendant).
...
sameTree(X, Y) :- descendantOf(X, Y); descendantOf(Y, X); descendantOf(X, Parent), descendantOf(Y, Parent).
...
drum(Nod, Nod, [Nod]).
drum(Nod1, Nod2, [Nod1, Nod2]) :- arc(Nod1, Nod2).
...
cost(Nod1, Nod2, N) :- arc(Nod1, Copil), descendantOf(Nod2, Copil), cost(Copil, Nod2, NDec), NDec is N - 1.
cost(Nod1, Nod2, N) :- arc(Parent, Nod1), cost(Parent, Nod2, NInc), NInc is N + 1.
