...
(define (all-palindromes-logical? n Bases)
  (or (null? Bases)
      (and (palindrome? (num->base n (car Bases)))
           (all-palindromes-logical? n (cdr Bases)))))
...
(define (count-bases n base)
  (cond ((= base 1) 0)
        ((palindrome? (num->base n base)) (+ 1 (count-bases n (sub1 base))))
        (else (count-bases n (sub1 base)))
))

(define (find-first start b)
  (if (>= (count-bases start 10) b)
      start
      (find-first (add1 start) b))
)
