...
insertElem BSTNil x = NewTree x BSTNil BSTNil
insertElem (NewTree elem l r) x
  | x < elem = NewTree elem (insertElem l x) r
  | otherwise = NewTree elem l (insertElem r x)

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil x = Nothing
findElem (NewTree elem l r) x
  | elem == x = Just elem
  | elem < x = findElem r x
  | otherwise = findElem l x

inorder :: BST a -> [a]
inorder BSTNil = []
inorder (NewTree elem l r) = inorder l ++ [elem] ++ inorder r

size :: BST a -> Int
size BSTNil = 0
size (NewTree elem l r) = 1 + size l + size r

height :: BST a -> Int
height BSTNil = 0
height (NewTree elem l r) = 1 + max (height l) (height r)

isBalanced :: (Ord a, Num a) => BST a -> Bool
isBalanced BSTNil = True
isBalanced (NewTree elem l r)
...
data Tree a = TreeNode a [Tree a] deriving (Eq, Show)
